...
namespace _3_GUI
{
    public partial class tudong : Form
    {
        ...
        //            .Select(o => o.First())
        //            .ToList();
        //        lstDay.ForEach(x => { loadDataNgay(x.Order.order_Time.ToString().Split(" ")[0]); });
        ...
        //        .Select(s => s.First())
        //        .Sum(x => x.Order.total_pay);
        //    var countOrder = _lstItem.Where(x => x.Order.order_Time.ToString()
        //            .Split(" ")[0] == time)
        //        .ToList().Count;
        ...
        {
            loadDGV();
            DateTime tgDateTime = DateTime.Now;
            string ngay = tgDateTime.Day.ToString();

            var lstDetail = _lstItem
                .Where(x => x.Order.order_Time.Day.ToString()
                    == ngay)
                .GroupBy(i => i.Order.order_Time)
                .Select(z => z.First())
                .ToList();
            ...
            dgvDoanhThu.Rows.Add(tgDateTime, countOrder, countProduct, oderFall, countpay);
        ...
        void senderexcel()
        {
            ...
            string sql = null;
            MyConnection = new System.Data.OleDb.OleDbConnection((@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source='C:\Users\MyPC\Documents\Book2.xlsx';Extended Properties=Excel 12.0 Xml;"));
            MyConnection.Open();
            myCommand.Connection = MyConnection;
       

            sql = "INSERT INTO [sheet1$] values(@thoigian,@sodonbanduoc,@sosanphambanduoc,@sodonhuy,@doanhthu)";
            for (int i = 0; i < dgvDoanhThu.Rows.Count; i++)
            {
                myCommand.Parameters.AddWithValue("@thoigian", Convert.ToString(dgvDoanhThu.Rows[i].Cells["Thời gian"].Value));
                //myCommand.CommandText = sql;
                myCommand.Parameters.AddWithValue("@sodonbanduoc", Convert.ToString(dgvDoanhThu.Rows[i].Cells["Số đơn bán được"].Value));
                //  myCommand.CommandText = sql;
                myCommand.Parameters.AddWithValue("@sosanphambanduoc", Convert.ToString(dgvDoanhThu.Rows[i].Cells["Số Sp bán được"].Value));
                // myCommand.CommandText = sql;
                myCommand.Parameters.AddWithValue("@sodonhuy", Convert.ToString(dgvDoanhThu.Rows[i].Cells["Số đơn hủy"].Value));
                // myCommand.CommandText = sql;
                myCommand.Parameters.AddWithValue("@doanhthu", Convert.ToString(dgvDoanhThu.Rows[i].Cells["Doanh thu"].Value));
                //  myCommand.CommandText = sql;
